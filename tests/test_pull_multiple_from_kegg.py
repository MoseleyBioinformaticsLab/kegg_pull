...
def setup_and_teardown():
    ...
    mkdir(actual_urls_dir)

    yield actual_urls_dir, mock_output_dir

    # Tear down
    rmtree(mock_output_dir, ignore_errors=True)
    rmtree(actual_urls_dir, ignore_errors=True)
...
def test_pull_multiple_from_kegg(mocker, setup_and_teardown):
    ...
    actual_saved_files = [join(mock_output_dir, saved_file) for saved_file in sorted(listdir(mock_output_dir))]
    expected_saved_files = [join(mock_output_dir, f'{entry_id}.txt') for entry_id in expected_successful_entry_ids]

    assert actual_saved_files == expected_saved_files

    expected_file_contents = [f'{entry_id} content' for entry_id in expected_successful_entry_ids]

    for (actual_saved_file, expected_file_content) in zip(actual_saved_files, expected_file_contents):
...
def _mock_input_output() -> tuple:
    ...
    mock_get_urls = [GetKEGGurl(entry_ids=entry_ids) for entry_ids in mock_entry_ids]
...
def _save_actual_url(actual_url: str, actual_urls_dir: str):
    ...
    url_file_path = join(actual_urls_dir, url_file_name)
...
def _test_actual_urls(actual_urls_dir: str, expected_urls: set):
    ...
    for url_file in listdir(actual_urls_dir):
        file_path: str = join(actual_urls_dir, url_file)
...
def _test_mock_single_pull(actual_urls_dir: str, mock_single_pull: callable, mocker):
    ...
    with open(join(actual_urls_dir, file_name), 'r') as f:
