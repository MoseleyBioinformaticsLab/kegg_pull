...
Usage:
    ...
    kegg_pull rest conv --conv-target=<target-database> <entry-ids> [--test] [--output=<output>]
    kegg_pull rest link <target-database> <source-database> [--test] [--output=<output>]
    kegg_pull rest link --link-target=<target-database> <entry-ids> [--test] [--output=<output>]
...
Options:
    ...
    --conv-target=<target-database>         The outside or KEGG database from which to view equivalent versions of the provided entry IDs. If a KEGG database, the provided entry IDs must be from an outside database and vice versa.
    ...
    <target-database>                       The name of the database to find cross-references in the source database.
    <source-database>                       The name of the database from which cross-references are found in the target database.
    --link-target-<target-database>         The name of the database to find cross-references in the provided entry IDs.
...
def main():
    ...
    if args['info']:
        ...
        if args['--conv-target']:
            target_database: str = args['--conv-target']
            entry_ids: list = u.parse_input_sequence(input_source=entry_ids)
        ...
        if args['--link-target']:
            target_database: str = args['--link-target']
            entry_ids: list = u.parse_input_sequence(input_source=entry_ids)
