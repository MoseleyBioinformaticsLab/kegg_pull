...
class EntryIdsGetter:
    ...
    def __init__(self, kegg_rest: r.KEGGrest = None) -> None:
        """
        :param kegg_rest: Optional KEGGrest object for making requests to the KEGG REST API to get entry IDs (if a KEGGrest object is not provided, a KEGGrest object is created with the default values).
        """
        self._kegg_rest = kegg_rest if kegg_rest is not None else r.KEGGrest()

    def from_database(self, database_name: str) -> list:
        """ Gets the KEGG entry IDs of a given database.

        :param database_name: The KEGG database to get the entry IDs for.
    ...
    def from_file(file_path: str) -> list:
        """ Gets KEGG entry IDs that are listed in a file with one entry ID on each line.
    ...
    def from_keywords(self, database_name: str, keywords: list) -> list:
        """ Gets entry IDs from a database based on keywords searched in the entries.

        :param database_name: The name of the database to get entry IDs from.
    ...
    def from_molecular_attribute(
        ...
        """ Gets entry IDs from a database containing chemical entries based on one (and only one) of three molecular attributes of the entries.
