...
Usage:
    ...
    kegg_pull map <source-database-name> <target-database-name> [--deduplicate] [--add-glycans] [--add-drugs] [--output=<output>]
    kegg_pull map entry-ids <entry-ids> <target-database-name> [--reverse] [--output=<output>]
    kegg_pull map <source-database-name> <intermediate-database-name> <target-database-name> [--deduplicate] [--add-glycans] [--add-drugs] [--output=<output>]

Options:
    -h --help                       Show this help message.
    <source-database-name>          The name of the database with entry IDs mapped to the target database.
    <target-database-name>          The name of the database with entry IDs mapped from the source database.
    ...
    <intermediate-database-name>    The name of an intermediate database with which to find cross-references to cross-references e.g. "kegg_pull map ko reaction compound" creates a mapping from ko-to-compound via ko-to-reaction cross-references connected to reaction-to-compound cross-references
"""
import docopt as d
...
def main():
    ...
    intermediate_database_name: str = args['<intermediate-database-name>']
    ...
    if args['entry-ids']:
        ...
        mapping: dict = kmap.entries_map(
            entry_ids=entry_ids, target_database_name=target_database_name, reverse=reverse)
    elif intermediate_database_name:
        mapping: dict = kmap.indirect(
            source_database_name=source_database_name, intermediate_database_name=intermediate_database_name,
            target_database_name=target_database_name, deduplicate=deduplicate, add_glycans=add_glycans, add_drugs=add_drugs)
    else:
        mapping: dict = kmap.database_map(
            source_database_name=source_database_name, target_database_name=target_database_name, deduplicate=deduplicate,
