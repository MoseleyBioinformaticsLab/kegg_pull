...
test_data = [
    (['entry-ids', '-'], {'n_tries': None, 'time_out': None, 'sleep_time': None}, 'u.parse_input_sequence', {'input_source': '-'},
     'SingleProcessMultiplePull', {'output': '.', 'unsuccessful_threshold': None}, {'force_single_entry': False, 'entry_field': None}),
    (['entry-ids', '1,2', '--output=out-dir/', '--sleep-time=10.1'], {'n_tries': None, 'time_out': None, 'sleep_time': 10.1},
     'u.parse_input_sequence', {'input_source': '1,2'}, 'SingleProcessMultiplePull', {'output': 'out-dir/', 'unsuccessful_threshold': None},
     {'force_single_entry': False, 'entry_field': None}),
    (['entry-ids', '1,2', '--n-tries=4', '--time-out=50', '--entry-field=mol'], {'n_tries': 4, 'time_out': 50, 'sleep_time': None},
     'u.parse_input_sequence', {'input_source': '1,2'}, 'SingleProcessMultiplePull', {'output': '.', 'unsuccessful_threshold': None},
     {'force_single_entry': False, 'entry_field': 'mol'}),
    (['entry-ids', '-', '--entry-field=mol'], {'n_tries': None, 'time_out': None, 'sleep_time': None},
     'u.parse_input_sequence', {'input_source': '-'}, 'SingleProcessMultiplePull', {'output': '.', 'unsuccessful_threshold': None},
     {'force_single_entry': False, 'entry_field': 'mol'}),
    (['database', 'pathway', '--output=out-dir', '--multi-process', '--sleep-time=20', '--force-single-entry'],
     ...
     {'output': 'out-dir', 'n_workers': None, 'unsuccessful_threshold': None}, {'force_single_entry': True, 'entry_field': None}),
    (['database', 'brite', '--multi-process', '--n-tries=5', '--time-out=35', '--n-workers=6'],
     ...
     {'output': '.', 'n_workers': 6, 'unsuccessful_threshold': None}, {'force_single_entry': True, 'entry_field': None}),
    (['entry-ids', '-', '--ut=0.4'], {'n_tries': None, 'time_out': None, 'sleep_time': None},
     'u.parse_input_sequence', {'input_source': '-'}, 'SingleProcessMultiplePull', {'output': '.', 'unsuccessful_threshold': 0.4},
     {'force_single_entry': False, 'entry_field': None})]


@pt.mark.parametrize(
    'args,kegg_rest_kwargs,entry_ids_method,entry_ids_kwargs,multiple_pull_class,multiple_pull_kwargs,pull_kwargs', test_data)
def test_main(
        mocker, _, args: list, kegg_rest_kwargs: dict, entry_ids_method: str, entry_ids_kwargs: dict, multiple_pull_class: str,
        multiple_pull_kwargs: dict, pull_kwargs: dict):
    ...
    multiple_pull_mock = mocker.MagicMock(pull=mocker.MagicMock(return_value=pull_result_mock))
