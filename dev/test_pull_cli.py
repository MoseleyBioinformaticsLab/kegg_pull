...
test_data = [
    (['entry-ids', '-'], {'n_tries': None, 'time_out': None, 'sleep_time': None}, {'output': '.', 'multiprocess_lock_save': False},
    ...
    (['entry-ids', '1,2', '--output=out-dir/', '--sleep-time=10.1'], {'n_tries': None, 'time_out': None, 'sleep_time': 10.1},
     {'output': 'out-dir/', 'multiprocess_lock_save': False}, 'u.parse_input_sequence', {'input_source': '1,2'}, 'SingleProcessMultiplePull',
     {'unsuccessful_threshold': None}, {'force_single_entry': False, 'entry_field': None}),
    ...
    (['entry-ids', '-', '--entry-field=mol'], {'n_tries': None, 'time_out': None, 'sleep_time': None},
     {'output': '.', 'multiprocess_lock_save': False}, 'u.parse_input_sequence', {'input_source': '-'}, 'SingleProcessMultiplePull',
     {'unsuccessful_threshold': None}, {'force_single_entry': False, 'entry_field': 'mol'}),
    (['database', 'pathway', '--output=out-dir', '--multi-process', '--sleep-time=20', '--force-single-entry'],
     {'n_tries': None, 'time_out': None, 'sleep_time': 20}, {'output': 'out-dir', 'multiprocess_lock_save': False}, 'ei.from_database',
    ...
    (['database', 'brite', '--multi-process', '--n-tries=5', '--time-out=35', '--n-workers=6'],
     {'n_tries': 5, 'time_out': 35, 'sleep_time': None}, {'output': '.', 'multiprocess_lock_save': False}, 'ei.from_database',
     {'database': 'brite'}, 'MultiProcessMultiplePull', {'n_workers': 6, 'unsuccessful_threshold': None},
     {'force_single_entry': True, 'entry_field': None}),
    (['entry-ids', '-', '--ut=0.4'], {'n_tries': None, 'time_out': None, 'sleep_time': None},
     {'output': '.', 'multiprocess_lock_save': False}, 'u.parse_input_sequence', {'input_source': '-'}, 'SingleProcessMultiplePull',
     {'unsuccessful_threshold': 0.4}, {'force_single_entry': False, 'entry_field': None})]


@pt.mark.parametrize(
    'args,kegg_rest_kwargs,single_pull_kwargs,entry_ids_method,entry_ids_kwargs,multiple_pull_class,' 'multiple_pull_kwargs,pull_kwargs',
    test_data)
def test_main(
