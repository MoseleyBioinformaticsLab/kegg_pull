...
def test_main_help(mocker):
    u.assert_main_help(mocker=mocker, module=po_cli, subcommand='pathway-organizer')
...
test_main_data = [
    ...
    (
        ['pathway-organizer', '--tln=node1,node2', '--fn=node3'],
        {'top_level_nodes': {'node1', 'node2'}, 'filter_nodes': {'node3'}}, None
    ),
    (
        ['pathway-organizer', '--tln=-'], {'top_level_nodes': {'node1', 'node2', 'node3'}, 'filter_nodes': None}, 'node1\nnode2\nnode3'
    ),
    (
        ['pathway-organizer', '--fn=-'], {'top_level_nodes': None, 'filter_nodes': {'node1', 'node2', 'node3'}},
        'node1\nnode2\nnode3'
    ),
    (
        ['pathway-organizer', '--tln=node1,node2,node3'], {'top_level_nodes': {'node1', 'node2', 'node3'}, 'filter_nodes': None},
        None
    ),
    (
        ['pathway-organizer', '--fn=node1,node2,node3'], {'top_level_nodes': None, 'filter_nodes': {'node1', 'node2', 'node3'}},
        None
    ),
...
def test_main_print(mocker, args: list, kwargs: dict, stdin_mock: str):
    ...
    u.test_main_print(
        ...
        method_kwargs=kwargs, module=po_cli, expected_output=expected_output
    )
...
def _get_mock_pathway_org_and_expected_output(mocker):
    ...
    expected_output: str = j.dumps(hierarchy_nodes_mock, indent=2)
